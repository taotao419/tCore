...
use crate::logger::{self, info, info2, warn};
...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.start_timing = get_time_ms();
    ...
    fn mark_current_exited(&self) {
        ...
        let running_span = get_time_ms() - inner.tasks[current].start_timing;
        logger::info2(
    ...
    fn find_next_task(&self) -> Option<usize> {
        ...
        (current + 1..current + self.num_app + 1)
            .map(|id| id % self.num_app) //try to get num_app_id via mod
            .find(|id| inner.tasks[*id].task_status == TaskStatus::Ready) //fetch first Ready Status task
    }
